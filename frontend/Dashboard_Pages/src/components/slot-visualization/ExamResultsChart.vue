...
let data = store.examResults
if (!data || typeof data.passed !== 'number' || typeof data.failed !== 'number') {
  data = { passed: 70, failed: 30 } // fallback mock data
}
console.log('ExamResultsChart data:', data)
const totalApplicants = computed(() => (data?.passed ?? 0) + (data?.failed ?? 0))
...
watch(() => store.examResults, (newVal) => {
  data = newVal
  createOrUpdateChart()
}, { deep: true })
