  ...
  <transition name="fade">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-0 min-h-[340px] w-full flex flex-col">
      ...
      <div class="flex-1 px-6 pb-6">
        <div v-if="loading" class="h-[200px] flex items-center justify-center" aria-live="polite">Loading...</div>
...
let data = store.socioeconomic
if (!data || typeof data.lower !== 'number' || typeof data.middle !== 'number' || typeof data.upper !== 'number') {
  data = { lower: 40, middle: 45, upper: 15 }
}
console.log('SocioEconomicChart data:', data)
const totalApplicants = computed(() => (data?.lower ?? 0) + (data?.middle ?? 0) + (data?.upper ?? 0))
        ...
        tooltip: {
          callbacks: {
            label: function(context) {
              ...
              const total = totalApplicants.value
...
watch(() => store.socioeconomic, (newVal) => {
  data = newVal
  createOrUpdateChart()
}, { deep: true })
</script>
